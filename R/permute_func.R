...
permute_integ <- function(betas, sds, mafs, dfs, alt_props, tol=1e-3, par_size=0, perm_par_size=0, density_list=NULL, perm_densities=T){
  ...
  if(perm_par_size>0){
    ...
    res <- parallel::parLapply(cl=clust, 1:ncol(betas),
                               ...
                               alt_props=alt_props,tol=tol,par_size=par_size,density_list=density_list,perm_densities=perm_densities)
    parallel::stopCluster(clust)
  # sequential version
  # run estimate_config for each permuted dataset
  } else res <- lapply(1:ncol(betas), function(j)
                        permute_once(betas,sds,mafs,dfs,alt_props,perm_col=j,tol,par_size,density_list,perm_densities))
